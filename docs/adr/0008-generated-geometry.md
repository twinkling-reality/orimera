...
- Date: 2026-09-03
- Deciders: Orimera build. Three independent proposals (admit as a segregated appearance layer;
...
`orimera.reconstruction`, the `.opm` container or any new container, the artifact table, the rung
...
4. **The badge has no render site.** Art. 50 requires a visible Generated badge. The rung itself is
   ...
   (orimera/ingest/formation.py:414). A badge would be satisfied by sequencing rather than by code.
...
- **"There is nothing to read origin from" is not a ground, but the repair is weaker than it
  ...
  generator's output against Orimera's own rung 3 point map rather than by asking the generator,
...
- **D5.** Stage and model registration: integer-only params, `deterministic = false`, a reviewed
  ...
  mask, because none does. Origin is decided by Orimera's visibility cull of the generator's output
...
- **D7.** Its own container and its own renderer entity, never inside OPM/1: no pick id, excluded
...
2. **A segregated, disclosed, non-citable, non-navigable appearance layer conditioned on one
   ...
   naming a supersession. Its sound parts are kept as checklist material: origin by Orimera's own
...
3. **A walkable generated room in a sandbox scene outside the Atlas.** Rejected: walking is the
   ...
   atlas-spatial-architecture.md:45; and "the stronger Orimera demonstration is not 'AI generated a
