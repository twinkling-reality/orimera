...
The demonstration has to run without an operator from the submission deadline on **2026-10-30** to
the end of judging on **2026-12-15**, roughly **46 days**. Nobody looks at the application in
November. Every choice in this document is shaped by that gap rather than by steady-state operations.
...
| Who performs the weekly check through 2026-12-15 | **OPEN**, section 9 |
...
| --- | --- | --- |
| Browser client | Static host (Vercel Hobby by default), no server side rendering, no serverless functions | The client is a build artifact. Giving it a runtime would add a component that can die during judging in exchange for nothing |
...
| Reconstruction (structure from motion plus splat or point map training) | Nebius Serverless Job on a GPU flavour, preemptible, checkpointed | Minutes to hours. **Never in the live path.** Scenes are reconstructed once, before submission, and the results are static assets |
...
container co-located with the API on a Serverless endpoint, scores marginally better against the
track's encouragement to deploy with Serverless Endpoints. It also puts the database on the least
reliable component in the stack. The platform terms for Serverless AI state that there is no service
level provided and that it "does not provide automatic retry, recovery, or redundancy mechanisms",
and that "infrastructure failures may result in workload failure with no automatic recovery".
Typical endpoint lifetime is documented as hours to days. Against a 46 day unattended window that is
the wrong component to hold the only copy of the data. Volume mounting is documented for Serverless
**jobs** and is undocumented for endpoints, so the endpoint variant additionally rests on an
undocumented assumption about where the data would even live.
...
The compliance sentence is identical under both placements, because a Compute virtual machine is
still Nebius AI Cloud. Serverless **Jobs** are retained for reconstruction, perception and batch
ingest, which is where a self terminating per second billing model is a genuine fit and where failure
is retryable, and that use also satisfies the track's encouragement to use Serverless Jobs for
...
unreasonable, it is optimising a different variable: submission-time alignment with a stated track
preference, rather than survival across an unattended judging window. This document optimises for
...
**ASSUMPTION.** That a preview grade service can be relied on for the judging window at all. This is
...
| --- | --- |
| Render free tier, whose web services spin down after 15 minutes idle and whose free PostgreSQL expires 30 days after creation | **VERIFIED** and fatal against a judging window that opens 32 days after the submission deadline. The database would expire before a judge ever loaded the page |
| Fly.io, technically fine and cheap | Not Nebius. The hard platform requirement is that the submission runs on Nebius Token Factory or Nebius AI Cloud, and Technological Implementation is a quarter of the score. Kept as an unused break-glass configuration only |
| Cloudflare R2 as the asset origin | Free egress would save roughly $2 to $10 across the project. Rejected because keeping the origin on Nebius keeps the platform compliance sentence literally true. An edge cache in front of the Nebius origin is compatible with this and is recommended |
...
judging window is never. Coupling them means every asset change forces a client redeploy and every
...
discount every other claim in the submission.
...
because judges may be a long way from eu-north1 and the first impression of the application is how
...
- **Fail closed at startup.** A missing required variable makes the process refuse to start, with the
  ...
  judging is strictly worse than one that never came up, because the monitoring in section 9 would
...
- **It must not claim more than it checks.** A `200` from `/healthz` says the process is alive. It
  ...
  it is why the weekly check through the judging window includes a catalog diff and not only a ping.
...
2026-08-31. That is two rounds in roughly ten weeks, and judging does not begin until December.

**ASSUMPTION:** that another round lands between feature freeze and the end of judging. Two rounds in
ten weeks is the observed cadence. This cannot be validated in advance, which is exactly why it is
mitigated structurally rather than watched for.

Without the preflight, the failure looks like this: a judge opens the demonstration on 2026-12-10,
the application calls a model identifier that was withdrawn three weeks earlier, and the request
returns a 404 class error with nobody present to notice. The demonstration is dead and the first
person to find out is a judge.
...
| Scheduled through the judging window | `orimera-preflight --json` | Feeds the weekly catalog diff described in section 9 |
...
2. **The embedding role has no same tier fallback.** It is the only embedding typed model in the
   ...
   candidate mitigation is to precompute and freeze every embedding before submission so the
...
**What the mitigations buy, stated honestly:** with fallbacks in place, a deprecation during judging
degrades answer quality rather than killing the demonstration. Degradation, not death. That is the
accurate description and it is the one to use in the submission.
...
at 15,000 tokens of context costs roughly $0.001 on the cheap reasoning tier. The confirmed baseline
of credits is ample for the corpus and the judging window. Inference is not where this project's
money goes.
...
**Reconstruction is a one time per scene cost.** Three to five scenes reconstructed once before
submission and cached as static assets is bounded and affordable. Reconstructing on demand during
judging is neither, and it is also forbidden by the rule that the hosted demonstration must not
depend on a long lived GPU job.
...
| Reconstruction GPU time, one time before submission | Bounded by scene count, and the line with the highest variance if the controls in 8.2 are not in place |
...
failure into either automatic recovery or an alert that reaches a person's phone. **VERIFIED:**
judging criteria are equally weighted across Technological Implementation, Design, Potential Impact
and Quality of the Idea, so a dead URL surrenders three of the four to whatever the video shows.
...
| Everything the automation misses | A named person loading the application, walking one scene and asking one question | Weekly through 2026-12-15 | Judgement |
...
| Host dies | None. This is the gap the platform does not fill | Redeploy from the one command path, restore the newest nightly dump | Hours of downtime and up to 24 hours of ingest data loss. Acceptable, because the corpus is static during judging and the dump holds it |
...
| **A non-Nebius host such as Fly.io** | Cheap and technically adequate | Does not satisfy the hard platform requirement. Retained only as an unused break-glass configuration | Low |
...
2. **The platform compliance sentence stays literally true.** The submission must run on Nebius Token
   Factory or Nebius AI Cloud. This disqualifies the non-Nebius row and is why the asset origin stays
   on Nebius even where an alternative would be marginally cheaper.
...
5. **Recovery is testable before submission.** An option whose failure path cannot be rehearsed in
   October is worth less than one that can, whatever its steady state numbers look like.
...
| The domain name | Registering it and pointing it | Before submission, and early enough that DNS and certificate issuance are not on the critical path on 2026-10-30 |
| Compute virtual machine or Serverless endpoint | The canary experiment's outage log, read against criterion 1 | At deployment rehearsal, not at submission |
...
| Who performs the weekly check | Asking a person and getting a yes | Before 2026-10-30 |

**A deployment rehearsal is scheduled before the deadline, not on it.** The entire plan above is
untested until the application has been deployed once, killed deliberately, and recovered from the
documented path. Everything in sections 6, 9 and 10 is a hypothesis until that has happened at least
once with a stopwatch running.
...
| D-10 | Nobody is named for the weekly check through 2026-12-15 | Asking a person |
