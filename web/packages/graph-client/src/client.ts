 ...
 * has been measured", so the server returns capture ids and this maps them through an injectable
 * function. The default is one island per capture, which is the honest reading for the corpus
 * that exists; changing it later is one argument rather than a server change.
 */
...
const oneIslandPerCapture: IslandOf = (captureId) => captureId as IslandIdRef;
...
export class OrimeraClient {
  ...
  constructor(options: ClientOptions) {
    ...
    this.#islandOf = options.islandOf ?? oneIslandPerCapture;
  }
...
export function adaptSnapshot(
  ...
  const entities: readonly EntityRecord[] = payload.entities.map(
    (row): EntityRecord => ({
      ...
      islandIds: row.capture_ids.map(islandOf),
  ...
  const occurrences: readonly OccurrenceRecord[] = payload.occurrences.map(
    (row): OccurrenceRecord => ({
      ...
      islandId: islandOf(row.capture_id),
